...
Use that REPL for quick probes while the app is running. For example, `{{top/ns}}.{{main/ns}}.core/app-state` is public so you can inspect or tweak Reagent state during debugging, but automated tests stay browser-first.
...
`verify` runs `clj-kondo`, a dev compile, dev E2E tests, and release E2E tests. Dev output writes to `public/js`; release output writes to `target/release/public/js`. Browser tests are Chromium-first; add mobile and broader browser coverage later when the app needs it.
